...
# Define Debug variable to be true
DEBUG = True


def update_teammateFeatures(driverID, newTeammateFeatures, db=dbDrivers):
        db.update_one({'driverID': driverID}, {'$push': {'teammateFeatures': newTeammateFeatures}}, upsert = True)


def update_teammateSet(driverID, newTeammate, db=dbDrivers):
    db.update_one({'driverID': driverID}, {'$push': {'teammateSet': newTeammate}}, upsert = True)
...
def main(): 
    ...
    # Initialize an empty set of teammates for each driver in the drivers data frame
    ...
    for season_rec in year_races_list: 
        ...
        for raceIdx in range(0, numRaces, 1):
            ...
            for constructor in raceResults['constructorID'].unique():
                ...
                constructordf = raceResults.loc[raceResults['constructorID'] == constructor]
                # print(constructordf)
                if DEBUG == True: 
                    print(f"DEBUG: Adding Teammates for the {constructordf.shape[0]} drivers for {constructor}")
                    
                # Get all the teammates in a list of dicts for teammate Features
                teammateEntry = [{'teammateID': teammateRow['driverID'], "team": constructor, 'year': year, 'race': raceIdx + 1} for _, teammateRow in constructordf.iterrows()]
                ...
                for driverIdx, driver in constructordf.iterrows():
                    ...
                    for teammateRecord in teammateEntry:
                        ...
                        if teammateRecord['teammateID'] != driver['driverID']: 
                            ...
                            driverRecord = dbDrivers.find_one({'driverID': driver['driverID']})
                            
                            # If this is a new teammate, then add them to the teammate set and record 
